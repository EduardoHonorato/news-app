<template>
    <div class="bg-white p-4 rounded shadow">
      ...
      <table class="w-full text-left table-auto">
        ...
        <tbody>
          <tr v-for="(country, index) in topCountries" :key="country.rank" class="border-b">
      ...
      <div class="text-center mt-4">
        <button @click="showModal = true" class="text-red-600 font-bold">
      ...
      <Modal :visible="showModal" @close="showModal = false">
        ...
        <table class="w-full text-left table-auto">
          ...
          <tbody>
            <tr v-for="(country, index) in fullMedalData" :key="country.rank" class="border-b">
              <td class="py-2">{{ country.rank }}</td>
              <td class="py-2 flex items-center">
                <img :src="getFlagUrl(convertCode(country.country.code))" alt="" class="h-4 w-6 mr-2">
                {{ country.country.name }}
  ...
  // Mapa de conversão de códigos de três letras para códigos de duas letras
